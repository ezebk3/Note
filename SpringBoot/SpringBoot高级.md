# springboot高级
 ...
 ![springboot高级](SpringBoot%E9%AB%98%E7%BA%A7.assets/OG-Spring.png) 
...
- Expiry
  ...
  ![](SpringBoot%E9%AB%98%E7%BA%A7.assets/%E5%9B%BE%E7%89%871.png)
...
- Spring提供了各种xxxCache的实现；如RedisCache，EhCacheCache ,
  ...
  ![Spring缓存抽象](SpringBoot%E9%AB%98%E7%BA%A7.assets/%E5%9B%BE%E7%89%872.png)
...
3. 整合MyBatis操作数据库
   ...
   #打印sql
   logging.level.{自己mapper的类路径}=debug
...
```JAVA
@MapperScan("com.chenhui.springboot.mapper")
...
- cacheNames/value
  ...
  - 是数组的方式，可以指定多个缓存
- key
  - 缓存数据使用的key；默认是使用方法参数的值  (1-方法的返回值)
  - 编写SpEL； 
    - `#id;参数id的值   `
    - `#a0  #p0  #root.args[0]`
- keyGenerator
...
![1606049577757](SpringBoot%E9%AB%98%E7%BA%A7.assets/1606049577757.png)

效果：

![1606049533406](SpringBoot%E9%AB%98%E7%BA%A7.assets/1606049533406.png)
...
![1606049760218](SpringBoot%E9%AB%98%E7%BA%A7.assets/1606049760218.png)
...
去RedisAutoConfiguration粘贴一个配置过来：
...
public class MyRedisConfig {
    ...
    public RedisTemplate<Object, Employee> empRedisTemplate(RedisConnectionFactory redisConnectionFactory) {
        ...
        //设置Jackson2JsonRedisSerializer序列器
...
方式一：
 ...
 ![在这里插入图片描述](SpringBoot%E9%AB%98%E7%BA%A7.assets/20200316115221373.png) 

Spring官方推荐我们自定义一个RedisCacheConfiguration来设置Redis配置了

[源码分析](https://blog.csdn.net/qq_41534573/article/details/104895896)
...
```java
    ...
    public RedisCacheConfiguration empRedisCacheConfiguration() {
        ...
        //拿到序列化k-v序列化对子，用于传给serializeValuesWith()
        RedisSerializationContext.SerializationPair<Employee> serializationPair = RedisSerializationContext.SerializationPair
                .fromSerializer(jackson2JsonRedisSerializer);

        //返回一个RedisCacheConfiguration
        return RedisCacheConfiguration.defaultCacheConfig().serializeValuesWith(serializationPair);
    }


    @Primary //将某个缓存模拟器作为主要CacheManager
    @Bean
...
```java
    @Bean
    public RedisCacheManager empCacheManager2(RedisConnectionFactory factory) {
        RedisSerializer<Department> jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer<>
                (Department.class);
...
[![img](SpringBoot%E9%AB%98%E7%BA%A7.assets/%E5%9B%BE%E7%89%873.png)
...
2. 流量削峰
   ...
   [![img](SpringBoot%E9%AB%98%E7%BA%A7.assets/%E5%9B%BE%E7%89%875.png)
...
3. topic
   ...
   ![img](SpringBoot%E9%AB%98%E7%BA%A7.assets/Snipaste_2020-04-09_14-11-13.png)
...
![img](SpringBoot%E9%AB%98%E7%BA%A7.assets/Snipaste_2020-04-11_23-25-26.png)
